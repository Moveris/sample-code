#!/usr/bin/env python3
"""
Moveris Live - Python WebSocket Client
...
class MoverisClient:
    ...
    def __init__(self, ws_url: str, secret_key: str, frame_rate: int = 10, quality: int = 70):
        """
                ...
                try:
                    ...
                    if msg_type == 'frame_received':
                        ...
                        if frame_num in self.send_times:
                            ack_latency = (receive_time - self.send_times[frame_num]) * 1000
                        ...
                        # Log every 50 frames
                        if frame_num % 50 == 0:
                            avg_ack = sum(self.ack_times) / len(self.ack_times) if self.ack_times else 0
                            logger.info(
                        ...
                        logger.info(f"🔄 {data.get('message', 'Processing started')}")
                        ...
                        logger.warning(f"⚠️ Server disconnecting: {data.get('reason', 'Unknown')}")
                        self.is_streaming = False
                        break
