        ...
        {authStage === 'liveliness' && (
          <div className="space-y-6">
            ...
            <div className="relative w-full max-w-3xl mx-auto pb-[56.25%] h-0 overflow-hidden rounded-xl shadow-lg">
              <video className="absolute top-0 left-0 w-full h-full" src='BigBuckBunny.mp4' loop autoPlay muted></video>
            </div>

            {/* Video Feed */}
            <div
              onClick={() => setExpanded((prev) => !prev)}
              className={`relative bg-gray-900 rounded-lg overflow-hidden cursor-pointer transition-all duration-500 ease-in-out ${
                expanded
                  ? "w-full max-w-5xl" // expanded size
                  : "w-40 h-20" // small preview size
              }`}
            >
              <video
                ref={videoRef}
